...
if (typeof Module["files"] !== "undefined" && Object.keys(Module["files"]).length > 0) {
	Module["preRun"] = function() {
		...
		for (let i = 0; i < names.length; i++) {
			...
			if (typeof ab == "string") {
				...
				xhr.open("GET", ab);
				xhr.onload = function() {
					if (Module["curfile"] == n) Module["curfile"] = undefined;
				...
				xhr.onprogress = function(e) {
					if (typeof Module["curfile"] == "undefined") Module["curfile"] = n;
					if (Module["setStatus"] && Module["curfile"] == n) Module["setStatus"](n + " (" + e.loaded + "/" + e.total + ")")
				};
	...
	Module["manifest"] = man;
	if (window.location.hash != "") Module["manifest"] = window.location.hash.substring(1)
}
if (!Module["arguments"]) {
	Module["arguments"] = [];
	...
	if (qstrings != "") {
		...
		for (let i = 0; i < qstring.length; i++) {
			if (qstring[i] == "-manifest") man = undefined;
			if ((qstring[i] == "+sv_port_rtc" || qstring[i] == "+connect" || qstring[i] == "+join" || qstring[i] == "+observe" || qstring[i] == "+qtvplay") && i + 1 < qstring.length) {
				...
				i++
			} else if (!document.referrer) Module["arguments"] = Module["arguments"].concat(qstring[i])
		}
	}
	if (Module["manifest"] != undefined) Module["arguments"] = Module["arguments"].concat(["-manifest", Module["manifest"]]);
...
function createWasm() {
	...
	function receiveInstance(instance, module) {
		...
		Module["asm"] = exports;
		wasmMemory = Module["asm"]["Ec"];
		updateGlobalBufferAndViews(wasmMemory.buffer);
		wasmTable = Module["asm"]["Fc"];
...
var ASM_CONSTS = {
	...
	247529: function($0, $1) {
		try {
			if (navigator.registerProtocolHandler) navigator.registerProtocolHandler(UTF8ToString($0), document.location.origin + document.location.pathname + "?%s" + document.location.hash, UTF8ToString($1))
		} catch (e) {}
	},
	1027692: function($0) {
...
function autoResumeAudioContext(ctx, elements) {
  ...
  function playAudio() {
    var audio = new Audio('audiofile.mp3');  // Substitua pelo seu arquivo de áudio
    audio.play().then(() => {
  ...
  // Escuta os eventos de interação do usuário
        ...
        element.addEventListener(event, function() {
          if (ctx.state === "suspended") {
            ...
            ctx.resume().then(() => {
              ...
              playAudio(); // Chama a função de tocar o áudio
            }).catch((error) => {
...
// Exemplo de como usar a função autoResumeAudioContext com o AudioContext
var audioContext = new (window.AudioContext || window.webkitAudioContext)();
autoResumeAudioContext(audioContext, [document.getElementById("canvas")]);
...
function _emscriptenfte_abortmainloop(fname, fatal) {
	...
	if (Module["stackTrace"]) throw "oh noes! something bad happened in " + fname + "!\n" + Module["stackTrace"]();
...
function _emscriptenfte_async_wget_data2(url, ctx, onload, onerror, onprogress) {
	...
	http.onload = function(e) {
		if (http.status == 200) {
			if (onload) wasmTable.get(onload)(ctx, _emscriptenfte_buf_createfromarraybuf(http.response))
		} else {
...
function _emscriptenfte_buf_delete(name) {
	...
	if (f) {
		...
		if (Module["cache"]) Module["cache"].delete("/_/" + name);
		return 1
	}
	return 0
}
...
function _emscriptenfte_buf_pushtolocalstore(handle) {
	...
	try {
		if (b.n.endsWith(".pak") || b.n.endsWith(".pk3") || b.n.endsWith(".kpf") || b.n.indexOf("/dlcache/") > 0) {
			...
			} else console.log("cache not available")
...
function _emscriptenfte_buf_rename(oldname, newname) {
	...
	if (Module["cache"]) {
		Module["cache"].match("/_/" + oldname).then(oldresp => {
			if (oldresp === undefined) Module["cache"].delete("/_/" + newname);
			else {
...
function _emscriptenfte_gl_loadtexturefile(texid, widthptr, heightptr, dataptr, datasize, fname, dopremul, genmips) {
	...
	img.onload = function() {
		...
		if (dopremul) GLctx.pixelStorei(GLctx.UNPACK_PREMULTIPLY_ALPHA_WEBGL, false);
...
function _emscriptenfte_pcm_loadaudiofile(ctx, callback, dataptr, datasize, snd_speed) {
	const successcb = function(buffer) {
		...
		for (let c = 0; c < buffer.numberOfChannels; c++) {
			...
			for (let f = 0; f < frames; f++) dst[f * chans + c] = 16384 * src[f]
		}
		wasmTable.get(callback)(ctx, outptr, frames, chans, rate);
		_free(outptr)
	};
	const failurecb = function(buffer) {
		wasmTable.get(callback)(ctx, 0, 0, 0, 0)
	};
	try {
	...
	if (gamepads !== undefined) {
		for (let i = 0; i < gamepads.length; i += 1) {
			...
			for (let j = 0; j < gp.buttons.length; j += 1) {
				...
				let p;
				if (typeof b == "object") p = b.pressed || b.value > .5;
				else p = b > .5;
				if (b.lastframe != p) {
					b.lastframe = p;
					wasmTable.get(FTEC.evcb.jbutton)(gp.index, j, p, gp.mapping == "standard")
				}
			}
			for (let j = 0; j < gp.axes.length; j += 1) wasmTable.get(FTEC.evcb.jaxis)(gp.index, j, gp.axes[j], gp.mapping == "standard")
		}
	}
	if (FTEC.xrsession != null && FTEC.xrframe != null && FTEC.referenceSpace != null) {
		...
		if (pose) {
			...
			if (count) {
				...
				quat.z /= count;
				quat.w /= count;
				wasmTable.get(FTEC.evcb.jorientation)(-3, org.x, org.y, org.z, quat.x, quat.y, quat.z, quat.w)
			}
		}
		for (const is of FTEC.xrsession.inputSources) {
			if (is === undefined || is == null) continue;
			let idx;
			if (is.handedness == "right") idx = -1;
			else if (is.handedness == "left") idx = -2;
			else if (is.handedness == "none" && (is.targetRayMode == "gaze" || is.targetRayMode == "screen")) idx = -4;
			else continue;
			...
			if (targetRayPose) {
				...
				wasmTable.get(FTEC.evcb.jorientation)(idx, org.x, org.y, org.z, quat.x, quat.y, quat.z, quat.w)
			}
			const gp = is.gamepad;
			if (gp == null) continue;
			if (gp.mapping != "xr-standard") continue;
			for (let j = 0; j < gp.buttons.length; j += 1) {
				const b = gp.buttons[j];
				let p;
				p = b.pressed;
				if (b.lastframe != p) {
					b.lastframe = p;
					wasmTable.get(FTEC.evcb.jbutton)(idx, j, p, gp.mapping == "standard")
				}
			}
			for (let j = 0; j < gp.axes.length; j += 1) {
				wasmTable.get(FTEC.evcb.jaxis)(idx, j, gp.axes[j], gp.mapping == "standard")
			}
		}
	}
}
...
function _emscriptenfte_setupcanvas(nw, nh, evresize, evmouse, evmbutton, evkey, evfile, evcbufadd, evjbutton, evjaxis, evjorientation, evwantfullscreen) {
	try {
		...
		window.onresize = function() {
			...
			if (FTEC.evcb.resize != 0) wasmTable.get(FTEC.evcb.resize)(Module["canvas"].width, Module["canvas"].height, scale)
		};
		window.onresize();
		...
		try {
			if (!FTEC.wakelock && navigator.wakeLock) navigator.wakeLock.request("screen").then(value => {
				FTEC.wakelock = value;
				value.addEventListener("release", () => {
					FTEC.wakelock = null
				})
...
function _emscriptenfte_setupmainloop(fnc) {
	...
	} else if (Module["close"]) Module["close"]();
	else if (Module["quiturl"]) document.location.replace(Module["quiturl"]);
	else {
		try {
...
function _emscriptenfte_xr_geteyeinfo(maxeyes, eyeptr) {
	if (!FTEC.xrframe || !FTEC.xrsession) return 0;
	const pose = FTEC.xrframe.getViewerPose(FTEC.referenceSpace);
	if (!pose) return 0;
	const layer = FTEC.xrsession.renderState.baseLayer;
	var e = 0;
	if (!FTEC.xrfbo) FTEC.xrfbo = GL.framebuffers.length;
...
function _emscriptenfte_xr_isactive() {
	...
	if (FTEC.xrsession != null) {
		ret |= 1;
		const pose = FTEC.xrframe.getViewerPose(FTEC.referenceSpace);
		if (pose.views.length) ret |= 2
	}
	return ret
}

function _emscriptenfte_xr_issupported() {
	if (navigator.xr) return FTEC.xrsupport;
	return 0
}

function _emscriptenfte_xr_setup(mode) {
	if (mode == -3) mode = FTEC.xrsession != null ? -1 : -2;
	if (mode == -2) {
		if (FTEC.xrsupport & 1 << 2) mode = 2;
		else if (FTEC.xrsupport & 1 << 1) mode = 1;
		else if (FTEC.xrsupport & 1 << 0) mode = 0
	}
	if (mode == -1) _emscriptenfte_xr_shutdown();
		...
		navigator.xr.requestSession(modes[mode], {
			...
			if (mode == 0) madecompatible();
			else Module.ctx.makeXRCompatible().then(madecompatible).catch(e => {
...
var asmLibraryArg = {
	...
	"_": _alGetString,
	"lc": _alIsBuffer,
	"bb": _alIsExtensionPresent,
	"_a": _alIsSource,
	"mc": _alListenerf,
	"Z": _alListenerfv,
	"$a": _alSourcePlay,
	"Y": _alSourceQueueBuffers,
	"Za": _alSourceStop,
	"ab": _alSourceUnqueueBuffers,
	"o": _alSourcef,
	"Ya": _alSourcefv,
	"w": _alSourcei,
	"hb": _alcCaptureCloseDevice,
	"lb": _alcCaptureOpenDevice,
	"jb": _alcCaptureSamples,
	"kb": _alcCaptureStart,
	"ib": _alcCaptureStop,
	"ua": _alcCloseDevice,
	"cb": _alcCreateContext,
	"gb": _alcDestroyContext,
	"Qb": _alcGetContextsDevice,
	"Rb": _alcGetCurrentContext,
	"Ob": _alcGetEnumValue,
	"Pb": _alcGetError,
	"$": _alcGetIntegerv,
	"db": _alcGetProcAddress,
	"x": _alcGetString,
	"eb": _alcIsExtensionPresent,
	"va": _alcMakeContextCurrent,
	"fb": _alcOpenDevice,
	"Tb": _alcProcessContext,
	"Sb": _alcSuspendContext,
	"za": _clock,
	"Nb": _emscripten_alcDevicePauseSOFT,
	"Mb": _emscripten_alcDeviceResumeSOFT,
	"Lb": _emscripten_alcGetStringiSOFT,
	"Jb": _emscripten_alcResetDeviceSOFT,
	"P": _emscripten_asm_const_int,
	"Ub": _emscripten_memcpy_big,
	"Wb": _emscripten_resize_heap,
	"gc": _emscripten_run_script_string,
	"Zb": _emscripten_thread_sleep,
	"ba": _emscriptenfte_abortmainloop,
	"jc": _emscriptenfte_al_loadaudiofile,
	"Ac": _emscriptenfte_alert,
	"Bc": _emscriptenfte_async_wget_data2,
	"aa": _emscriptenfte_buf_create,
	"xc": _emscriptenfte_buf_delete,
	"pc": _emscriptenfte_buf_getsize,
	"oc": _emscriptenfte_buf_open,
	"nc": _emscriptenfte_buf_pushtolocalstore,
	"rc": _emscriptenfte_buf_read,
	"sc": _emscriptenfte_buf_release,
	"wc": _emscriptenfte_buf_rename,
	"qc": _emscriptenfte_buf_write,
	"Vb": _emscriptenfte_gl_loadtexturefile,
	"tc": _emscriptenfte_openfile,
	"hc": _emscriptenfte_pcm_loadaudiofile,
	"qb": _emscriptenfte_polljoyevents,
	"zc": _emscriptenfte_print,
	"nb": _emscriptenfte_rtc_candidate,
	"Cc": _emscriptenfte_rtc_create,
	"Aa": _emscriptenfte_rtc_offer,
	"sb": _emscriptenfte_settitle,
	"Ea": _emscriptenfte_setupcanvas,
	"mb": _emscriptenfte_setupmainloop,
	"rb": _emscriptenfte_updatepointerlock,
	"yc": _emscriptenfte_uptime_ms,
	"uc": _emscriptenfte_window_location,
	"y": _emscriptenfte_ws_close,
	"ca": _emscriptenfte_ws_connect,
	"S": _emscriptenfte_ws_recv,
	"D": _emscriptenfte_ws_send,
	"pb": _emscriptenfte_xr_geteyeinfo,
	"Da": _emscriptenfte_xr_isactive,
	"ob": _emscriptenfte_xr_issupported,
	"u": _emscriptenfte_xr_setup,
	"Ca": _emscriptenfte_xr_shutdown,
	"vc": _emscritenfte_buf_enumerate,
	"Xb": _environ_get,
	...
	"ga": _glCopyTexImage2D,
	"zb": _glCopyTexSubImage2D,
	"xb": _glCreateProgram,
	"ub": _glCreateShader,
	"Qa": _glCullFace,
	"K": _glDeleteBuffers,
...
var ___wasm_call_ctors = Module["___wasm_call_ctors"] = function() {
	return (___wasm_call_ctors = Module["___wasm_call_ctors"] = Module["asm"]["Gc"]).apply(null, arguments)
};
var _free = Module["_free"] = function() {
	return (_free = Module["_free"] = Module["asm"]["Hc"]).apply(null, arguments)
};
var _malloc = Module["_malloc"] = function() {
	return (_malloc = Module["_malloc"] = Module["asm"]["Ic"]).apply(null, arguments)
};
var _main = Module["_main"] = function() {
	return (_main = Module["_main"] = Module["asm"]["Jc"]).apply(null, arguments)
};
var _emscripten_GetAlcProcAddress = Module["_emscripten_GetAlcProcAddress"] = function() {
	return (_emscripten_GetAlcProcAddress = Module["_emscripten_GetAlcProcAddress"] = Module["asm"]["Kc"]).apply(null, arguments)
};
var __get_tzname = Module["__get_tzname"] = function() {
	return (__get_tzname = Module["__get_tzname"] = Module["asm"]["Lc"]).apply(null, arguments)
};
var __get_daylight = Module["__get_daylight"] = function() {
	return (__get_daylight = Module["__get_daylight"] = Module["asm"]["Mc"]).apply(null, arguments)
};
var __get_timezone = Module["__get_timezone"] = function() {
	return (__get_timezone = Module["__get_timezone"] = Module["asm"]["Nc"]).apply(null, arguments)
};
